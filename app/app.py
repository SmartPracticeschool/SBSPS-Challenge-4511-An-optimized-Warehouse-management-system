...
def index():
    ...
    #     y2, m2, d2 = form.dt.data.strftime('%Y-%m-%d').split('-')
    #     today = date.today()
    #     y1, m1, d1 = today.strftime("%Y-%m-%d").split('-')
    #     dt1 = Date(int(d1), int(m1), int(y1))
    #     dt2 = Date(int(d2),int(m2), int(y2)) 
...
def select():
    int_a = [int(x) for x in request.form.values()]
    print(type(int_a[0]))
    print(int_a)
    if(int_a[0] == 1):
        ...
        return render_template('puri.html')
    
    elif(int_a[0] == 2):
        
        return render_template('bbsr.html')
...
def predict():
    ...
    dt2 = Date(int(d2),int(m2), int(y2)) 
    print(getDifference(dt1, dt2), "days")
    days_final = int(getDifference(dt1, dt2))
    # final_features = [np.array(int_features)]
    # prediction = model.predict(final_features)
    
    warnings.filterwarnings("ignore")
    if int_features[1]!= 0 or int_features[2]!= 0:
        int1 = days_final
        int2 = int(int_features[1])
        print(type(int_features[2]))
        print(int2)
        store_number = (int2%10)+1
        print(store_number)
        int3 = int(int_features[2])
        print(int3)
        if (int2 >= 1) and (int2<=9):
            url_val = 'puri.html'
            store_name = 'Puri'
        elif int2 >=10 and int2 <20:
            url_val = 'bbsr.html'
            store_name = 'Bhubaneshwar'
        else:
            url_val = 'delhi.html'
            store_name = 'Delhi'
            
        print(url_val)
        x,d= backend(int1, int2, int3)
        print(d)
        print(x)
        pred=model.predict(read_data1(x),d)
        ...
        y = pred[:d]
        y_1 =()
        for i in range(days_final):
            y_1 = y_1 + (y[i],)
        pred = pred[:d][d-1]
        print(pred)
        product = {108696:'Meat', 164088: 'Dairy', 804098: 'Sea Food', 1695936: 'Fruit', 2032088 : 'Flour'}
        # output = round(prediction[0], 2)
        bar = create_plot(days_final, y_1)
        if(int3 == 164088):
            return render_template(url_val, prediction_text='The demand prediction for '+str(product[int3])+' in '+store_name+' store '+str(store_number)+' on date '+str(int_features[0])+' = {} ltrs'.format(pred), plot=bar)
        else:
            return render_template(url_val, prediction_text='The demand prediction for '+str(product[int3])+' in '+store_name+' store '+str(store_number)+' on date '+str(int_features[0])+' = {} kg'.format(pred), plot=bar)
